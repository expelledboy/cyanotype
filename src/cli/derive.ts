...
export const loadDerivedCompose = (
    ...
    if ((e as NodeJS.ErrnoException).code === "ENOENT") {
      throw {
        ...
        hint:
          ...
          `run the cyanotype derive CLI against your compose file or manifests, pointing --out ` +
          `at this path, before the suite runs.`,
  ...
  try {
    ...
    throw {
      ...
      cause,
      hint:
        `"${path}" is not valid JSON. It is generated by the cyanotype derive CLI — re-run ` +
        `derive rather than hand-editing, and check nothing truncated the file.`,
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw {
      ...
      hint:
        `"${path}" parsed as JSON but is not an object. The derive CLI writes a map of ` +
        `component key to adapter config; re-run derive rather than hand-editing.`,
    } satisfies DerivedComposeInvalidError;
  }
    ...
    if (!result.success) {
      throw {
        ...
        hint:
          ...
          `cause.issues for the exact fields. If you wrote the derive script yourself, it is ` +
          `emitting a shape the adapter does not accept; if it came from the cyanotype derive ` +
          `CLI, re-run it against the current compose file.`,
      } satisfies DerivedComposeInvalidError;
    }
  }
  ...
  if (missing.length > 0) {
    throw {
      ...
      missing,
      hint:
        `"${path}" is missing [${missing.join(", ")}]. Your env.ts declares these as ` +
        `expectedKeys so a missing entry fails here rather than as a confusing ` +
        `service-not-found later. Either your derive step did not emit them — check the ` +
        `component names it walks match these keys — or the key list is stale.`,

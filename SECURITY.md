...
Cyanotype is a **pre-1.0 developer preview**. No formal security guarantees,
...
- GitHub: open a [private security advisory](https://github.com/expelledboy/cyanotype/security/advisories/new)
...
Cyanotype executes local shell commands as part of its core function —
...
- Running cyanotype against untrusted Blueprints, test code, or
  `--kubeconfig` / `DOCKER_HOST` endpoints you do not control. Treat
  cyanotype the same way you would treat a local test runner with shell
...
In-scope reports include: privilege escalation through cyanotype's own
...
from a user's environment through cyanotype's logging or event streams.
